...
class Parser {
    private:
        ...
        Node::Expr *parse_term() {
            ...
            if (look().has_value() && look().value().type == TokenType::_int_lit) {
                ...
                expr->var = int_lit;
                return expr;
            } else if (look().has_value() &&
                    look().value().type == TokenType::_bool_lit) {
                ...
                return expr;
                    ...
                    look().value().type == TokenType::_open_par) {
                mov_next();   // Skip '('
                expr = parse_expr();
        ...
        Node::Expr *parse_bin_expr(int min_prec = 0) {
            ...
            while (look().has_value() && look().value().type == TokenType::_bin_op &&
                    get_prec(look().value()) >= min_prec) {
        ...
        Node::Stmnt *parse_stmnt() {
            ...
            if (look().has_value() && look().value().type == TokenType::_exit) {
                if (look(1).has_value() && look(1).value().type == TokenType::_open_par) {
                    ...
                    if (look().has_value() &&
                            look().value().type == TokenType::_close_par) {
                        mov_next(); // Skip ')'
                    } else {
                ...
                check_semi_col();
                    ...
                    look().value().type == TokenType::_type_dec &&
                    look(1).has_value() &&
                    look(1).value().type == TokenType::_ident &&
                ...
                stmnt_node->var = if_block;
                    ...
                    look().value().type == TokenType::_close_curl) {
                check_semi_col(false);
                return stmnt_node;
            } else if (look().has_value() &&
                    look().value().type == TokenType::_open_curl) {
            ...
            } else if (look().has_value() && look().value().type == TokenType::_break){
                if (loop_count == 0){
                    std::cerr << "Break statement not within loop\n";
                    exit(-1);
                } 
                Node::Break* break_stmnt = allocator.alloc<Node::Break>();
                break_stmnt->token = mov_next().value();
                stmnt_node->var = break_stmnt;
                check_semi_col(true);
            } else {
    ...
    public:
        Parser(const std::vector<Token> &tokens)
            : tokens(tokens), tokens_len(tokens.size()),
            allocator(1024 * 1024 * 5) {};
